...
from typing import Callable, Dict, List, Tuple, Any, Union, Optional, Iterator, Set
...
def estimate_task_memory(shape: Tuple[int, ...], dtype: np.dtype) -> float:
    """
    Estimate memory requirements for a task based on data shape and type.
    
    Args:
        shape: Shape of the data to process
        dtype: Data type
    ...
    # Estimate assuming we need 3x the data size for processing
    return num_elements * element_size * 3


def estimate_num_workers(total_memory: float, memory_per_worker: float) -> int:
    """
    Estimate the number of workers based on available memory.
    ...
    """
    return max(1, int(total_memory / memory_per_worker))
